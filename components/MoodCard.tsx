...
export const MoodCard: React.FC<MoodCardProps> = ({ data, isMe, isShared = false }) => {
  ...
  // Me = Right
  // Partner = Left
  let alignClass = isMe ? 'ml-auto' : 'mr-auto';
  let roundedClass = isMe ? 'rounded-tr-none' : 'rounded-tl-none';
  
  if (isShared) {
      alignClass = 'mx-auto';
      roundedClass = 'rounded-[1.5rem]'; // Fully rounded for shared items
  }
  ...
  return (
    <div className={`relative group w-[95%] max-w-sm mb-6 ${alignClass} ${style.rotation} transition-transform hover:z-10`}>
       ...
       <div className={`flex w-full bg-white border-4 border-black rounded-[1.5rem] ${roundedClass} shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden`}>
          ...
          {/* Layout: Icon on Left for Partner/Shared, Right for Me */}
          {/* If Shared, we stick to standard Left-Icon layout but centered container */}
          <div className={`flex w-full ${isMe && !isShared ? 'flex-row-reverse' : 'flex-row'}`}>
            
            {/* Icon Column */}
            <div className={`w-[80px] shrink-0 ${bgColor} ${isMe && !isShared ? 'border-l-4' : 'border-r-4'} border-black flex flex-col items-center justify-center p-2 relative`}>
